...
const newsAll = document.querySelector('.news__all');
const newsItems = document.querySelector('.news__items');

newsAll.addEventListener('click',function(){
    newsItems.classList.toggle('more')
    if(newsItems.classList.contains('more')){
        newsAll.textContent = 'показать меньше'
    }else{
        newsAll.textContent = 'показать все'
        document.querySelector('.news--page').scrollIntoView({ behavior: 'smooth' });
    }
})
